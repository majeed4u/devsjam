...
export const FormCreatableSelect = ({
  ...
  useEffect(() => {
    const subscription = form.watch((formValues) => {
      ...
      } else if (isMulti && Array.isArray(fieldValue)) {
        // Find matching options for multi-select
        const matched = (fieldValue as string[])
          .map((val) => options.find((opt) => opt.value === val))
          .filter((opt): opt is SelectOption => opt !== undefined);
        setValue(matched);
      } else if (!isMulti && typeof fieldValue === "string") {
        // Find matching option for single select
        const matched = options.find((opt) => opt.value === fieldValue);
        setValue(matched ?? null);
      }
    });
    return () => subscription.unsubscribe();
  }, [form, name, isMulti, options]);
